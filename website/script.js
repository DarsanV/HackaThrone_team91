...
function initializeApp() {
    // Clear any mock/default reports on app initialization
    // This ensures only real challan-generated reports are shown
    clearMockReports();
...
// Clear mock/default reports that weren't generated by actual challans
function clearMockReports() {
    // Only clear if there's a flag indicating these are mock reports
    // Or if the app is being loaded for the first time
...
function loadRecentReports() {
    // Clear any default/mock reports on first load
    const savedReports = JSON.parse(localStorage.getItem('snapnearn_recent_reports') || '[]');
    
    // If there are no real reports, show empty state
    if (savedReports.length === 0) {
        const reportsContainer = document.getElementById('recentReports');
        reportsContainer.innerHTML = `
            <div class="no-reports">
